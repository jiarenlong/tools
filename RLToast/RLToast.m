...
- (id)initWithText:(NSString *)text {
    if (self = [super init]) {
        ...
        _contentView.backgroundColor = [UIColor colorWithRed:0.2f green:0.2f blue:0.2f alpha:0.75f];
...
+ (void)toastWithText:(NSString *)text {
    [RLToast toastWithText:text bottomOffset:80 duration:DEFAULT_DISPLAY_DURATION];
}

+ (void)toastWithText:(NSString *)text duration:(CGFloat)duration {
    RLToast *toast = [[RLToast alloc] initWithText:text];
    [toast setDuration:duration];
    [toast show];
}

+ (void)toastWithText:(NSString *)text topOffset:(CGFloat)topOffset {
    [RLToast toastWithText:text topOffset:topOffset duration:DEFAULT_DISPLAY_DURATION];
}

+ (void)toastWithText:(NSString *)text topOffset:(CGFloat)topOffset duration:(CGFloat)duration {
    RLToast *toast = [[RLToast alloc] initWithText:text];
    [toast setDuration:duration];
    [toast showFromTopOffset:topOffset];
}

+ (void)toastWithText:(NSString *)text bottomOffset:(CGFloat)bottomOffset {
    [RLToast toastWithText:text bottomOffset:bottomOffset duration:DEFAULT_DISPLAY_DURATION];
}

+ (void)toastWithText:(NSString *)text bottomOffset:(CGFloat)bottomOffset duration:(CGFloat)duration {
    RLToast *toast = [[RLToast alloc] initWithText:text];
    [toast setDuration:duration];
